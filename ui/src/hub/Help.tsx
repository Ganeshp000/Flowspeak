...
const TIPS: { emoji: string; title: string; body: string }[] = [
  {
    ...
    body: "Press and hold your dictation key (Fn by default), speak naturally, then release. FlowSpeak transcribes on-device — nothing leaves your Mac unless you choose a cloud cleanup engine.",
  },
  {
    emoji: "",
  ...
  },
  {
    ...
    body: 'Open Dictionary and add names, jargon, or acronyms it keeps mishearing. Add the correct spelling plus any "also heard as" variants and FlowSpeak will fix them automatically.',
  },
  {
    emoji: "",
...
export function Help() {
  return (
    <div style={{ maxWidth: 720 }}>
      <PageTitle sub="A few tips to get the most out of FlowSpeak.">Help</PageTitle>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {TIPS.map((t) => (
          <Card key={t.title}>
            <div style={{ display: "flex", gap: 14 }}>
              {t.emoji && <div style={{ fontSize: 22, lineHeight: 1.2 }}>{t.emoji}</div>}
              <div>
                <div
                  style={{
                    fontFamily: font.ui,

//! Windows platform layer for FlowSpeak: a low-level keyboard hook for
...
use flowspeak_core::{AsrEngine, CleanupContext, CleanupMode, CleanupProvider, StatsSummary};
...
fn support_dir() -> std::path::PathBuf {
    ...
    std::path::PathBuf::from(base).join("FlowSpeak")
}
...
fn emit_bar(state: &'static str) {
    if let Some(app) = APP.get() {
        ...
        let _ = app.emit_to(OVERLAY_LABEL, "flowspeak://flowbar/state", P { state });
    }
}
...
fn current_settings_inner() -> flowspeak_core::Settings {
    SETTINGS
        .get()
    ...
    let run_local = || -> Option<anyhow::Result<String>> {
        LOCAL.get().and_then(|m| {
            m.lock().unwrap().as_mut().map(|w| {
                let messages = flowspeak_core::cleanup::build_messages(&raw_norm, &ctx);
                w.cleanup(&messages)
            })
        })
    };
...
fn on_ptt_down() {
    ...
    let _ = now_ms();
    emit_bar("recording");
    std::thread::spawn(|| match flowspeak_audio::start(|_: &[f32]| {}) {
        Ok(handle) => {
            *CAPTURE.get_or_init(|| Mutex::new(None)).lock().unwrap() = Some(handle);
        }
        Err(e) => eprintln!("[flowspeak:win] mic capture failed: {e}"),
    });
}
...
fn on_ptt_up() {
    ...
    std::thread::spawn(move || {
        ...
        let Some(res) = handle.and_then(|h| h.stop()) else {
            eprintln!("[flowspeak:win] no audio captured");
            return;
        };
        let was_real_attempt = res.duration_secs() >= 0.6;
        let peak = res.samples.iter().fold(0f32, |m, &s| m.max(s.abs()));
        if peak < 0.005 {
            eprintln!("[flowspeak:win] ⚠ audio is silent — the mic isn't being captured");
            if was_real_attempt {
                if let Some(app) = APP.get() {
                    crate::diag::report(app, flowspeak_core::InjectionFailure::NoAudioCaptured);
                }
            }
            return;
        }
        let Some(asr) = ASR.get().cloned() else {
            eprintln!("[flowspeak:win] ASR not ready (model still loading or missing)");
            if was_real_attempt && ASR_MODEL_MISSING.load(Ordering::SeqCst) {
                if let Some(app) = APP.get() {
                    crate::diag::report(app, flowspeak_core::InjectionFailure::AsrUnavailable);
                }
            }
            return;
        };
            ...
            Ok(t) => {
                ...
                if !text.is_empty() {
                    if let Err(e) = paste_text(&text) {
                        eprintln!("[flowspeak:win] paste failed: {e}");
                        if let Some(app) = APP.get() {
                            crate::diag::report(app, flowspeak_core::InjectionFailure::ClipboardUnavailable);
                        }
                    } else {
                    ...
                    if let Some(app) = APP.get() {
                        crate::diag::report(app, flowspeak_core::InjectionFailure::EmptyTranscript);
                    }
                }
            }
            Err(e) => {
                eprintln!("[flowspeak:win] ASR error: {e}");
                if was_real_attempt {
                    if let Some(app) = APP.get() {
                        crate::diag::report(app, flowspeak_core::InjectionFailure::AsrUnavailable);
                    }
...
/// global hook closes) doesn't require relaunching FlowSpeak.
    ...
    std::thread::spawn(|| unsafe {
        ...
        let hook = loop {
            match SetWindowsHookExW(WH_KEYBOARD_LL, Some(hook_proc), hinst, 0) {
                Ok(h) => break h,
                Err(_) => {
                    eprintln!("[flowspeak:win] failed to install keyboard hook — retrying…");
                    if !reported {
                        if let Some(app) = APP.get() {
        ...
        if reported {
            eprintln!("[flowspeak:win] keyboard hook recovered — Right Ctrl is live now.");
            crate::diag::clear_last_error();
        }
...
pub fn install(app: AppHandle) {
    ...
    let _ = SETTINGS.set(Mutex::new(flowspeak_core::Settings::load(&settings_path())));
    let _ = DICTIONARY.set(Mutex::new(flowspeak_core::DictionaryStore::load(&dict_path())));
    ...
    std::thread::spawn(|| {
        ...
        if !path.exists() {
            eprintln!("[flowspeak:win] ASR model not found at {}", path.display());
        ...
        match flowspeak_asr::WhisperEngine::load(&path) {
            Ok(engine) => {
                ...
                eprintln!("[flowspeak:win] ASR ready");
            }
            Err(e) => {
                eprintln!("[flowspeak:win] ASR load failed: {e}");
                ASR_MODEL_MISSING.store(true, Ordering::SeqCst);
            }
        }
    });
    ...
    spawn_hook_thread();
    eprintln!("[flowspeak:win] keyboard hook installed (push-to-talk: Right Ctrl)");
}
...
pub fn rebuild_providers() {
    ...
    let key = keyring::Entry::new("com.flowspeak.app", "openai_api_key")
        .ok()
        .and_then(|e| e.get_password().ok())
        .filter(|k| !k.trim().is_empty());
    if let Some(slot) = OPENAI.get() {
        *slot.lock().unwrap() = key.map(|k| {
            flowspeak_cleanup::OpenAiProvider::with_base_url(k, model, Some(base_url))
        });
    }
}
...
pub fn stats_summary(tz_offset_minutes: i32) -> StatsSummary {
    STATS
        ...
        .unwrap_or_else(|| flowspeak_core::StatsStore::default().summary(tz_offset_minutes, unix_now()))
}

...
//! `flowspeak://flowbar/state`, so the tray demo items prove the event pipeline.
...
fn position_overlay(w: &WebviewWindow) {
    ...
    let Some(monitor) = monitor else {
        eprintln!("[flowspeak] no monitor found — overlay stays at default position");
        return;
    };
    ...
    eprintln!(
        "[flowspeak] overlay placed: monitor {}x{} @({},{}) scale {:.1} -> window {}x{} @({},{})",
...
#[tauri::command]
fn get_settings() -> flowspeak_core::Settings {
    hotkey::current_settings()
}

#[tauri::command]
fn set_settings(settings: flowspeak_core::Settings) {
...
#[tauri::command]
fn get_stats(tz_offset_minutes: i32) -> flowspeak_core::StatsSummary {
    hotkey::stats_summary(tz_offset_minutes)
}

/// Recent dictations for the Hub Home history list (newest first).
#[tauri::command]
fn get_history() -> Vec<flowspeak_core::HistoryItem> {
    hotkey::history(200)
}
...
fn request_microphone() {
    ...
    {
        std::thread::spawn(|| {
            if let Ok(h) = flowspeak_audio::start(|_: &[f32]| {}) {
...
/// just while FlowSpeak is frontmost): register + prompt, then open the pane.
#[tauri::command]
...
fn set_api_key(provider: String, key: String) -> Result<(), String> {
    ...
    let entry =
        keyring::Entry::new("com.flowspeak.app", account).map_err(|e| e.to_string())?;
    let key = key.trim();
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let open = MenuItem::with_id(app, "open", "Open FlowSpeak", true, None::<&str>)?;
            let demo_rec =
            ...
            let quit = MenuItem::with_id(app, "quit", "Quit FlowSpeak", true, None::<&str>)?;

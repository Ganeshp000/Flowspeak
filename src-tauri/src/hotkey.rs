...
//! key-up into the real [`flowspeak_core`] dictation state machine, and turns the
//! machine's actions into `flowspeak://flowbar/state` events the overlay pill
...
mod imp {
    ...
    use flowspeak_core::state::{Action, BarState};
    use flowspeak_core::{
        AsrEngine, CleanupContext, CleanupMode, CleanupProvider, Input, PipelineEvent, StateMachine,
        TriggerToken,
    };
    use flowspeak_ipc::BindingId;

    const OVERLAY_LABEL: &str = "flowspeak_bar";
    ...
    static CAPTURE: OnceLock<Mutex<Option<flowspeak_audio::CaptureHandle>>> = OnceLock::new();
    static ASR: OnceLock<Arc<flowspeak_asr::WhisperEngine>> = OnceLock::new();
    static OPENAI: OnceLock<Mutex<Option<flowspeak_cleanup::OpenAiProvider>>> = OnceLock::new();
    static ANTHROPIC: OnceLock<Mutex<Option<flowspeak_cleanup::AnthropicProvider>>> = OnceLock::new();
    static LOCAL: OnceLock<Mutex<Option<crate::local_llm::LocalWorker>>> = OnceLock::new();
    static SETTINGS: OnceLock<Mutex<flowspeak_core::Settings>> = OnceLock::new();
    static DICTIONARY: OnceLock<Mutex<flowspeak_core::DictionaryStore>> = OnceLock::new();
    static STATS: OnceLock<Mutex<flowspeak_core::StatsStore>> = OnceLock::new();
    ...
    fn support_dir() -> PathBuf {
        ...
        PathBuf::from(home).join("Library/Application Support/FlowSpeak")
    }
    ...
    pub fn record_dictation(text: &str, duration_secs: f32) {
        let words = flowspeak_core::stats::count_words(text);
        if words == 0 {
            return;
        }
    ...
    pub fn history(limit: usize) -> Vec<flowspeak_core::HistoryItem> {
        STATS
            .get()
            ...
            .map(|m| {
                m.lock()
                    ...
                    .map(|e| DictEntryDto {
                        ...
                        auto: matches!(e.source, flowspeak_core::DictSource::Auto),
                    })
                    .collect()
            })
            .unwrap_or_default()
    ...
    pub fn dictionary_add(correct: String, mishears: Vec<String>) {
        if let Some(m) = DICTIONARY.get() {
            let mut store = m.lock().unwrap();
            store.add(correct, mishears, flowspeak_core::DictSource::Manual);
    ...
    pub fn dictionary_learn(correct: String, mishears: Vec<String>) {
        if let Some(m) = DICTIONARY.get() {
            let mut store = m.lock().unwrap();
            store.add(correct, mishears, flowspeak_core::DictSource::Auto);
            let _ = store.save(&dict_path());
        }
    }
    ...
    pub fn stats_summary(tz_offset_minutes: i32) -> flowspeak_core::StatsSummary {
        STATS
            .get()
            .map(|m| m.lock().unwrap().summary(tz_offset_minutes, unix_now()))
            .unwrap_or_else(|| {
                flowspeak_core::StatsStore::default().summary(tz_offset_minutes, unix_now())
            })
    }
    ...
    fn read_key(account: &str, env_var: &str) -> Option<String> {
        ...
        keyring::Entry::new("com.flowspeak.app", account)
            .ok()
    ...
    pub fn current_settings() -> flowspeak_core::Settings {
        SETTINGS
            .get()
    ...
    pub fn update_settings(new: flowspeak_core::Settings) {
    ...
    pub fn rebuild_providers() {
        ...
        let openai = read_openai_key().map(|k| {
            flowspeak_cleanup::OpenAiProvider::with_base_url(
                k,
        ...
        let anthropic = read_anthropic_key()
            .map(|k| flowspeak_cleanup::AnthropicProvider::new(k, settings.anthropic_model.clone()));
        eprintln!(
            "[flowspeak] cleanup providers: openai={}, anthropic={}",
            openai.is_some(),
            anthropic.is_some()
        );
        match OPENAI.get() {
    ...
    fn clean_transcript(raw: &str) -> String {
        ...
        let raw_norm = flowspeak_core::cleanup::pre_normalize_layout(raw);
        let raw = raw_norm.as_str();
        let raw_out = flowspeak_core::cleanup::post_process(&raw_norm);
        ...
        let run_local = || -> Option<anyhow::Result<String>> {
            LOCAL.get().and_then(|m| {
                m.lock().unwrap().as_mut().map(|w| {
                    ...
                    let messages = flowspeak_core::cleanup::build_messages(raw, &ctx);
                    w.cleanup(&messages)
                })
            })
        };
        ...
        match result {
            Some(Ok(cleaned)) => {
                ...
                let cleaned = flowspeak_core::cleanup::post_process(&cleaned);
                if flowspeak_core::cleanup::evaluate_gates(&raw_out, &cleaned, level).passed() {
                    cleaned
                } else {
                    eprintln!("[flowspeak] cleanup gate rejected the edit — pasting raw");
                    raw_out
                }
            }
            Some(Err(e)) => {
                eprintln!("[flowspeak] cleanup failed ({e}) — pasting raw");
                raw_out
            }
            None => {
                if matches!(settings.cleanup_mode, CleanupMode::Local) {
                    eprintln!("[flowspeak] local cleanup model not wired yet — pasting raw");
                } else {
                    eprintln!("[flowspeak] cleanup provider has no API key — pasting raw");
                }
                raw_out
            }
        }
    ...
    fn emit_bar(app: &AppHandle, state: &'static str) {
        eprintln!("[flowspeak] pill -> {state}");
        let _ = app.emit_to(OVERLAY_LABEL, "flowspeak://flowbar/state", BarPayload { state });
    }
        ...
        match action {
            ...
            Action::StartCapture { .. } => {
                ...
                std::thread::spawn(move || {
                    ...
                    match flowspeak_audio::start(move |bars| {
                        let _ = app_cb.emit_to(
                            OVERLAY_LABEL,
                            "flowspeak://audio/waveform",
                        ...
                        Err(e) => eprintln!("[flowspeak] mic capture failed to start: {e}"),
                    }
                });
            }
            ...
            Action::StopCaptureAndFinalize { session } => {
                ...
                std::thread::spawn(move || {
                    ...
                    let Some(res) = handle.and_then(|h| h.stop()) else {
                        eprintln!("[flowspeak] no audio captured");
                        finish();
                        return;
                    };
                    let peak = res.samples.iter().fold(0f32, |m, &s| m.max(s.abs()));
                    eprintln!(
                        "[flowspeak] captured {} samples @ {} Hz (~{:.2}s), peak {:.4}",
                        res.samples.len(),
                        res.sample_rate,
                        res.duration_secs(),
                        peak
                    ...
                    if peak < 0.005 {
                        eprintln!(
                            "[flowspeak] ⚠ audio is silent — the mic isn't being captured. Grant \
                        ...
                        if was_real_attempt {
                            crate::diag::report(
                                &app2,
                                flowspeak_core::InjectionFailure::NoAudioCaptured,
                            );
                        }
                        finish();
                        return;
                    ...
                    let Some(asr) = ASR.get().cloned() else {
                        eprintln!("[flowspeak] ASR not ready (model still loading or missing)");
                        if was_real_attempt && ASR_MODEL_MISSING.load(Ordering::SeqCst) {
                            crate::diag::report(&app2, flowspeak_core::InjectionFailure::AsrUnavailable);
                        }
                        finish();
                        return;
                    };
                    let pcm = flowspeak_audio::resample_to_16k(&res.samples, res.sample_rate);
                    match asr.transcribe(&pcm) {
                        Ok(t) => {
                            let raw = t.text;
                            eprintln!("[flowspeak] TRANSCRIPT: \"{}\"", raw);
                            // Clean the transcript (cloud LLM if configured), then paste.
                            let text = clean_transcript(&raw);
                            if text != raw {
                                eprintln!("[flowspeak] CLEANED:   \"{}\"", text);
                            }
                            if !text.is_empty() {
                                if let Err(e) = crate::paste::paste_text(&text) {
                                    eprintln!("[flowspeak] paste failed: {e}");
                                    ...
                                    let failure = if !crate::paste::is_trusted() {
                                        flowspeak_core::InjectionFailure::AccessibilityNotGranted
                                    } else {
                                        flowspeak_core::InjectionFailure::ClipboardUnavailable
                                    };
                                ...
                                crate::diag::report(&app2, flowspeak_core::InjectionFailure::EmptyTranscript);
                            }
                            let _ = app2.emit_to(
                                OVERLAY_LABEL,
                                "flowspeak://transcript",
                                TranscriptPayload { text },
                            );
                        }
                        Err(e) => {
                            eprintln!("[flowspeak] ASR error: {e}");
                            if was_real_attempt {
                                crate::diag::report(&app2, flowspeak_core::InjectionFailure::AsrUnavailable);
                            }
                        }
    ...
    pub fn install(app: AppHandle) {
        ...
        std::thread::spawn(|| {
            let path = model_path();
            if !path.exists() {
                eprintln!("[flowspeak] ASR model not found at {}", path.display());
                ASR_MODEL_MISSING.store(true, Ordering::SeqCst);
                return;
            }
            match flowspeak_asr::WhisperEngine::load(&path) {
                Ok(engine) => {
                    let _ = ASR.set(Arc::new(engine));
                    eprintln!("[flowspeak] ASR model loaded — ready to transcribe");
                }
                Err(e) => {
                    eprintln!("[flowspeak] ASR model load failed: {e}");
                    ASR_MODEL_MISSING.store(true, Ordering::SeqCst);
                }
            }
        });
        ...
        let settings = flowspeak_core::Settings::load(&settings_path());
        let dict = flowspeak_core::DictionaryStore::load(&dict_path());
        eprintln!(
            "[flowspeak] cleanup mode: {:?}, level: {:?}",
            settings.cleanup_mode, settings.cleanup_level
        );
        let _ = SETTINGS.set(Mutex::new(settings));
        let _ = DICTIONARY.set(Mutex::new(dict));
        let _ = STATS.set(Mutex::new(flowspeak_core::StatsStore::load(&stats_path())));
        rebuild_providers();
        ...
        if crate::paste::is_trusted() {
            eprintln!("[flowspeak] Accessibility granted — Fn works in every app, paste enabled");
        } else {
            eprintln!(
                "[flowspeak] ⚠ Accessibility NOT granted — Fn only works while FlowSpeak is \
                 frontmost and paste is disabled. Prompting; grant FlowSpeak under System \
        ...
        eprintln!(
            "[flowspeak] (info) Input Monitoring: {}",
        ...
        std::thread::spawn(|| {
            ...
            eprintln!("[flowspeak] Accessibility present — creating global Fn tap");
            ...
            let port = loop {
                ...
                eprintln!(
                    "[flowspeak] Fn tap null despite Accessibility — likely a stale TCC entry from \
                     an earlier build. Run: tccutil reset Accessibility com.flowspeak.app, \
                     or toggle FlowSpeak off/on under System Settings → Privacy & Security → \
                     Accessibility. Retrying…"
                );
                if !reported {
                    if let Some(app) = APP.get() {
                        crate::diag::report(app, flowspeak_core::InjectionFailure::HotkeyTapFailed);
                    }
                    reported = true;
                }
            ...
            if reported {
                eprintln!("[flowspeak] Fn tap recovered — the key is live now.");
                crate::diag::clear_last_error();
            }
...
mod other {
    ...
    pub fn current_settings() -> flowspeak_core::Settings {
        flowspeak_core::Settings::default()
    }
    pub fn update_settings(_new: flowspeak_core::Settings) {}
    pub fn rebuild_providers() {}
    pub fn stats_summary(tz_offset_minutes: i32) -> flowspeak_core::StatsSummary {
        flowspeak_core::StatsStore::default().summary(tz_offset_minutes, 0)
    }
    pub fn history(_limit: usize) -> Vec<flowspeak_core::HistoryItem> {
        Vec::new()
    }

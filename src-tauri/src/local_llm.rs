...
impl LocalWorker {
    ...
    pub fn cleanup(
        &mut self,
        messages: &[flowspeak_core::cleanup::CleanupMsg],
...
/// Platform application-support dir: `~/Library/Application Support/FlowSpeak`
/// on macOS, `%APPDATA%\FlowSpeak` on Windows.
fn app_support_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    {
        let base = std::env::var("APPDATA").unwrap_or_default();
        PathBuf::from(base).join("FlowSpeak")
    }
...
pub fn worker_bin_path() -> Option<PathBuf> {
    let exe_name = if cfg!(target_os = "windows") {
        "flowspeak-llm-worker.exe"
    } else {
        "flowspeak-llm-worker"
    };
    ...
    {
        let home = std::env::var("HOME").unwrap_or_default();
        let dev = PathBuf::from(home).join("FlowSpeak/target/release/flowspeak-llm-worker");
...
pub fn spawn_default() -> Option<LocalWorker> {
    ...
    if !model.exists() {
        eprintln!("[flowspeak] local model not found at {}", model.display());
        return None;
    }
    match LocalWorker::spawn(&bin, &model) {
        Ok(w) => {
            eprintln!("[flowspeak] local LLM worker started ({})", bin.display());
            Some(w)
        }
        Err(e) => {
            eprintln!("[flowspeak] local LLM worker failed to start: {e}");
            None
        }
    }
}

...
[ -z "$APP" ] && APP="$(/usr/bin/find "$REPO_ROOT/target" -maxdepth 5 -path "*/release/bundle/macos/FlowSpeak.app" -print -quit 2>/dev/null)"
[ -z "$DMG" ] && DMG="$(/usr/bin/find "$REPO_ROOT/target" -maxdepth 5 -path "*/release/bundle/dmg/*.dmg" -print -quit 2>/dev/null)"

if [ -z "$APP" ] || [ ! -d "$APP" ]; then
  echo "No FlowSpeak.app found — build one first." >&2
  exit 1
fi
...
if [ -n "$DMG" ] && [ -f "$DMG" ]; then
  ...
  if hdiutil attach "$STAGE/download.dmg" -nobrowse -readonly -mountpoint "$MOUNTED" -quiet; then
    pass "the dmg mounts"
    DMG_APP="$(/usr/bin/find "$MOUNTED" -maxdepth 2 -name "FlowSpeak.app" -print -quit)"
    if [ -n "$DMG_APP" ]; then
      SOURCE_APP="$DMG_APP"
    else
      fail "no FlowSpeak.app inside the dmg"
    fi
  else
    MOUNTED=""
...
cp -R "$SOURCE_APP" "$STAGE/FlowSpeak.app"
INSTALLED="$STAGE/FlowSpeak.app"
...
LAUNCH_COPY="$STAGE/launch/FlowSpeak.app"
